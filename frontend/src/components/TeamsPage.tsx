...
export function TeamsAndPlayers() {
  ...
  const handleAddPlayer = () => {
    if (newPlayerName.trim() && selectedTeam && selectedSport) {
      ...
      // Update selected team to reflect new player
  ...
  const handleDeletePlayer = (playerName: string) => {
    if (window.confirm("Are you sure you want to remove this player?")) {
      ...
      // Update selected team to reflect removed player
      setSelectedTeam(prev => ({
        ...prev,
  ...
  const selectedSportData = sportsData.find(sport => sport.id === selectedSport);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-white text-4xl lg:text-5xl mb-2">Teams & Players</h1>
        ...
        {selectedSport && !selectedTeam && (
          <Dialog open={isAddTeamOpen} onOpenChange={setIsAddTeamOpen}>
            ...
            <DialogContent className="bg-slate-900 text-white border-slate-800">
              <DialogHeader>
                <DialogTitle>Add New Team to {selectedSportData?.name}</DialogTitle>
              </DialogHeader>
              <div className="space-y-4 mt-4">
                ...
                <Button onClick={handleAddTeam} className="w-full bg-purple-600 hover:bg-purple-700">
                  Add Team
                </Button>
              </div>
            </DialogContent>
          ...
          <Dialog open={isAddPlayerOpen} onOpenChange={setIsAddPlayerOpen}>
            ...
            <DialogContent className="bg-slate-900 text-white border-slate-800">
              ...
              <div className="space-y-4 mt-4">
                ...
                <Button onClick={handleAddPlayer} className="w-full bg-purple-600 hover:bg-purple-700">
                  Add Player
                </Button>
              </div>
            </DialogContent>
      ...
      {!selectedSport && (
        <div>
          <h2 className="text-white text-2xl mb-4 font-semibold">Select a Sport</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {sportsData.map(sport => {
              const totalPlayers = sport.teams.reduce((sum, team) => sum + team.players.length, 0);
              return (
                <Card
                  key={sport.id}
                ...
                >
                  <CardContent className="p-6">
                    ...
                    <p className="text-white text-2xl font-semibold mb-1">{sport.name}</p>
                    <p className="text-slate-400 text-sm">{totalPlayers} total players</p>
                  </CardContent>
                </Card>
              );
            })}
      ...
      {selectedSport && !selectedTeam && (
        <div>
          <h2 className="text-white text-2xl mb-4 font-semibold">
            {selectedSportData.name} Teams ({selectedSportData.teams.length})
          </h2>
          {selectedSportData.teams.length === 0 ? (
            <Card className="bg-slate-900/50 border-slate-800">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-slate-500">Click "Add Team" to create your first team</p>
              </CardContent>
            </Card>
          ) : (
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      >
                        ...
                        <div>
                          <p className="text-white text-xl font-semibold">{team.name}</p>
                    ...
                    <Badge variant="outline" className="text-slate-300 text-sm border-slate-600">
          ...
          {selectedTeam.players.length === 0 ? (
            <Card className="bg-slate-900/50 border-slate-800">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-slate-500">Click "Add Player" to add your first player</p>
              </CardContent>
            </Card>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {selectedTeam.players.map((player, index) => (
                <Card key={index} className="bg-slate-900/50 border-slate-800 backdrop-blur group hover:border-purple-600 transition-all">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-white text-lg font-medium">{player}</p>
